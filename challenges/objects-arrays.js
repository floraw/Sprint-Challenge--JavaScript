...
const getUni = function(array) {
  let newArray = [];
  for(i = 0; i < array.length; i++) {
    newArray.push(array[i].university);
  }
  newArray = newArray.sort();
  return newArray;
};

const universities = getUni(graduates);
...
const getContact = function(array) {
  let list = [];
  for(i = 0; i < array.length; i++) {
    let str = `${array[i].first_name} ${array[i].email} `;
    list.push(str);
  }
  return list;
}
...
const contactInfo = getContact(graduates);
...
function display(array) {
  ...
  array.forEach(function(item){
    nameList.push(`Name: ${item.animal_name}, Scientific: ${item.scientific_name}`);
  });
  return nameList;
};

const displayNames = display(zooAnimals);
...
*/
const populationTotal = zooAnimals.reduce((total, num) => total +num.population, 0);
